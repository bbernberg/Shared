...
#define kBackgroundColor [SHPalette backgroundColor]
...
@property (nonatomic, readonly) NSString *keychainPassCode;
@property (nonatomic, readonly) NSString *keychainTimerStart;
@property (nonatomic, readonly) NSString *userDefaultsKeyForTimerDuration;
@property (nonatomic, strong) UIImageView *sharedLogo;
...
- (NSString *)keychainPassCode {
  NSString *baseString = @"keychainPasscode";
  if (self.userID) {
    return [NSString stringWithFormat:@"%@_%@", baseString, self.userID];
  }
  return baseString;
}

- (NSString *)keychainTimerStart {
  NSString *baseString = @"keychainTimerStart";
  if (self.userID) {
    return [NSString stringWithFormat:@"%@_%@", baseString, self.userID];
  }
  return baseString;
...
- (NSString *)userDefaultsKeyForTimerDuration {
  NSString *baseString = @"timerDurationKey";
  if (self.userID) {
    return [NSString stringWithFormat:@"%@_%@", baseString, self.userID];
  }
  return baseString;
...
- (BOOL)passcodeExistsInKeychain {
  if (self.userID) {
    return [SFHFKeychainUtils getPasswordForUsername:self.keychainPassCode
...
- (CGFloat)timerDuration {
  if (self.userID) {
    return [[NSUserDefaults standardUserDefaults] floatForKey:self.userDefaultsKeyForTimerDuration];
  } else {
    return CGFLOAT_MAX;
  }
}
...
- (void)setTimerDuration:(float) duration {
  if (self.userID) {
    [[NSUserDefaults standardUserDefaults] setFloat:duration forKey:self.userDefaultsKeyForTimerDuration];
  }
}

- (CGFloat)timerStartTime {
  if (self.userID) {
    NSString *keychainValue = [SFHFKeychainUtils getPasswordForUsername:self.keychainTimerStart
                                                         andServiceName:kKeychainServiceName
                                                                  error:nil];
...
- (void)saveTimerStartTime {
  if (self.userID) {
    [SFHFKeychainUtils storeUsername:self.keychainTimerStart
...
- (BOOL)didPasscodeTimerEnd {
  if (self.userID) {
    ...
    if (now - [self timerStartTime] >= [self timerDuration] ||
        [self timerStartTime] == -1) {
      return YES;
    }
    return NO;
  } else {
...
- (void)deletePasscodeFromKeychain {
  if (self.userID) {
    [SFHFKeychainUtils deleteItemForUsername:self.keychainPassCode
                              andServiceName:kKeychainServiceName
                                       error:nil];
  }
}
...
- (void)viewDidLoad {
  ...
  self.sharedLogo = [[UIImageView alloc] initWithImage: [UIImage imageNamed:@"Shared_Name_Logo"]];
  self.sharedLogo.contentMode = UIViewContentModeScaleAspectFit;
  self.sharedLogo.frame = CGRectMake(60.f, 70.f, 200.f, 50.f);
  self.sharedLogo.hidden = YES;
  [self.view addSubview:self.sharedLogo];
	...
	_failedAttemptLabel.text = @"1 Security Code Failed Attempt";
  ...
  [_animatingView addSubview:_passcodeTextField];
	...
	_enterPasscodeLabel.text = _isUserChangingPasscode ? NSLocalizedString(@"Enter your old security code", @"") : NSLocalizedString(@"Enter your security code", @"");
	...
	[UIView animateWithDuration: kLockAnimationDuration animations: ^{
		if (_beingDisplayedAsLockScreen) {
			...
			if (_isUserTurningPasscodeOff) {
				[[LTHPasscodeViewController sharedUser] deletePasscodeFromKeychain];
			}
			// Update the Keychain if adding or changing passcode
			else {
				[SFHFKeychainUtils storeUsername:self.keychainPassCode
...
- (void)showForEnablingPasscodeInViewController:(UIViewController *)viewController {
  self.sharedLogo.hidden = YES;
	...
	self.title = NSLocalizedString(@"Enable Security Code", @"");
}
...
- (void)showForChangingPasscodeInViewController:(UIViewController *)viewController {
  self.sharedLogo.hidden = YES;
	...
	[self prepareNavigationControllerWithController: viewController];
	self.title = NSLocalizedString(@"Change Security Code", @"");
}
...
- (void)showForTurningOffPasscodeInViewController:(UIViewController *)viewController {
  self.sharedLogo.hidden = YES;
	...
	[self prepareNavigationControllerWithController: viewController];
	self.title = NSLocalizedString(@"Turn Code Off", @"");
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
	...
	if (typedString.length == 4) {
		NSString *savedPasscode = [SFHFKeychainUtils getPasswordForUsername:self.keychainPassCode
                                                         andServiceName:kKeychainServiceName
                                                                  error:nil];
...
- (void)resetUI {
	...
	if (_isUserConfirmingPasscode) {
		if (_isUserEnablingPasscode) _enterPasscodeLabel.text = NSLocalizedString(@"Re-enter your security code", @"");
		else if (_isUserChangingPasscode) _enterPasscodeLabel.text = NSLocalizedString(@"Re-enter your new security code", @"");
	}
	else if (_isUserBeingAskedForNewPasscode) {
		if (_isUserEnablingPasscode || _isUserChangingPasscode) {
			_enterPasscodeLabel.text = NSLocalizedString(@"Enter your new security code", @"");
		}
	}
	else _enterPasscodeLabel.text = NSLocalizedString(@"Enter your security code", @"");
}
...
- (void)resetUIForReEnteringNewPasscode {
	...
	NSString *savedPasscode = [SFHFKeychainUtils getPasswordForUsername:self.keychainPassCode
                                                       andServiceName:kKeychainServiceName
                                                                error:nil];
	_enterPasscodeLabel.text = savedPasscode.length == 0 ? NSLocalizedString(@"Enter your security code", @"") : NSLocalizedString(@"Enter your new security code", @"");
	
	_failedAttemptLabel.hidden = NO;
	_failedAttemptLabel.text = NSLocalizedString(@"Security codes did not match. Try again.", @"");
...
- (void)denyAccess {
  ...
  //													  userInfo: nil];
	...
	if (_failedAttempts == 1) _failedAttemptLabel.text = NSLocalizedString(@"1 Security Code Failed Attempt", @"");
	else {
		_failedAttemptLabel.text = [NSString stringWithFormat: NSLocalizedString(@"%i Security Code Failed Attempts", @""), _failedAttempts];
	}
...
- (void)applicationDidEnterBackground {
	if ([[LTHPasscodeViewController sharedUser] passcodeExistsInKeychain]) {
		...
		if ([[LTHPasscodeViewController sharedUser] timerDuration] == 0) {
...
- (void)applicationWillEnterForeground {
	if ([[LTHPasscodeViewController sharedUser] passcodeExistsInKeychain] &&
      [[LTHPasscodeViewController sharedUser] didPasscodeTimerEnd] &&
...
- (void)applicationWillResignActive {
	if ([[LTHPasscodeViewController sharedUser] passcodeExistsInKeychain]) {
		[[LTHPasscodeViewController sharedUser] saveTimerStartTime];
	}
}
